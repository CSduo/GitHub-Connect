  ...
  return (
    <div className="min-h-[100dvh] pb-24" style={{ background: "var(--bg)" }}>
      ...
      <div className="container-anv pt-4">
        <Link
          href="/"
          className="inline-flex items-center gap-2 font-ui text-sm transition-colors hover:text-[var(--gold)]"
      ...
      <article className="container-anv pt-6 pb-12">
        <div className="max-w-[var(--max-reader)] mx-auto">
          ...
          <h1
            className="font-display text-3xl md:text-5xl lg:text-6xl leading-[0.95] mt-3"
            style={{ color: "var(--ink)" }}
          >
            {article.title}
          </h1>
          ...
          {article.subtitle && (
            <p className="font-body text-lg md:text-xl mt-4" style={{ color: "var(--muted)" }}>
              {article.subtitle}
            </p>
          )}
          ...
          <div className="flex flex-wrap items-center gap-4 mt-6 pb-6" style={{ borderBottom: "1px solid var(--border)" }}>
            {article.authorName && (
              <span className="font-ui text-sm" style={{ color: "var(--ink)" }}>{article.authorName}</span>
            )}
            {article.publishedAt && (
          ...
          <div className="flex items-center gap-3 py-4">
            <button className="flex items-center gap-2 px-4 py-2 rounded-full font-ui text-xs font-medium transition-colors" style={{ background: "var(--surface-soft)", color: "var(--ink)", border: "1px solid var(--border)" }}>
          ...
          {article.keyTakeaways && article.keyTakeaways.length > 0 && (
            ...
            >
              ...
              <ul className="space-y-2">
                {article.keyTakeaways.map((tk: string, i: number) => (
                  <li key={i} className="font-body text-sm flex items-start gap-2" style={{ color: "var(--ink)" }}>
                    <span style={{ color: "var(--gold)" }}>{i + 1}.</span> {tk}
                  </li>
                ))}
              </ul>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {article.tags.map((tag: string) => (
                  <Link
                    ...
                    className="px-3 py-1 rounded-full font-ui text-xs transition-colors hover:bg-[var(--gold)] hover:text-[#1a1108]"
